...
#define LOGI(FORMAT, ...) __android_log_print(ANDROID_LOG_INFO,"dcc",FORMAT,##__VA_ARGS__);
#define LOGE(FORMAT, ...) __android_log_print(ANDROID_LOG_ERROR,"dcc",FORMAT,##__VA_ARGS__);
#define MAX_AUDIO_FRME_SIZE 48000 * 4

JNIEXPORT void JNICALL
Java_com_dcc_ndkapp1_MainActivity_decode(JNIEnv *env, jclass type, jstring input_,
                                         jstring outptu_) {

    //需要转码的视频文件(输入的视频文件)
    const char *input = env->GetStringUTFChars(input_, 0);
    const char *outptu = env->GetStringUTFChars(outptu_, 0);
    ...
    env->ReleaseStringUTFChars(outptu_, outptu);
}

JNIEXPORT void JNICALL
Java_com_dcc_ndkapp1_MainActivity_render(JNIEnv *env, jobject instance, jstring input_,
                                         jobject surface) {
    ...
    while (av_read_frame(pFormatCtx, packet) >= 0) {
        ...
        if (packet->stream_index == v_stream_idx) {
            ...
            if (got_picture) {
                ...
                avpicture_fill((AVPicture *) rgb_frame, (uint8_t *)out_buffer.bits, AV_PIX_FMT_RGBA,
                               pCodecCtx->width,
                               pCodecCtx->height);
...
JNIEXPORT void JNICALL
Java_com_dcc_ndkapp1_MainActivity_sound(JNIEnv *env, jobject instance, jstring input_,
                                            jstring output_) {
    const char *input = env->GetStringUTFChars(input_, 0);
 ...
 */
JNIEXPORT void JNICALL
Java_com_dcc_ndkapp1_MainActivity_play(JNIEnv *env, jobject instance, jstring input_) {
    const char *input = (env)->GetStringUTFChars( input_, 0);
